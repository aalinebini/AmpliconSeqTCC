...
========================================================================================
                         nf-core/ampliconseq
========================================================================================
 nf-core/ampliconseq Analysis Pipeline.
 #### Homepage / Documentation
 https://github.com/nf-core/ampliconseq
----------------------------------------------------------------------------------------
*/
...
def helpMessage() {
    ...
    Mandatory arguments:
      ...
      --key_file [file]               Path to file with primer names and primer sequences
      --sample_file [file]            Path to file with the name of the samples and fastq files for reverse and forward sequencing
...
//             .into { ch_read_files_fastq; ch_read_files_trimming }
// } else {
//     Channel
...
Channel.from(summary.collect{ [it.key, it.value] })
    ...
    .map { x -> """
    id: 'nf-core-ampliconseq-summary'
    description: " - this information is collected when the pipeline is started."
    section_name: 'nf-core/ampliconseq Workflow Summary'
...
workflow.onComplete {
    ...
    def subject = "[nf-core/ampliconseq] Successful: $workflow.runName"
    if (!workflow.success) {
        subject = "[nf-core/ampliconseq] FAILED: $workflow.runName"
    }
    def email_fields = [:]
    ...
    try {
        if (workflow.success) {
            ...
            if (mqc_report.getClass() == ArrayList) {
                log.warn "[nf-core/ampliconseq] Found multiple reports from process 'multiqc', will use only one"
                mqc_report = mqc_report[0]
            }
        }
    } catch (all) {
        log.warn "[nf-core/ampliconseq] Could not attach MultiQC report to summary email"
    }
    ...
    if (email_address) {
        try {
            ...
            log.info "[nf-core/ampliconseq] Sent summary e-mail to $email_address (sendmail)"
        } catch (all) {
            ...
            log.info "[nf-core/ampliconseq] Sent summary e-mail to $email_address (mail)"
        }
    }
    ...
    if (workflow.success) {
        log.info "-${c_purple}[nf-core/ampliconseq]${c_green} Pipeline completed successfully${c_reset}-"
    } else {
        checkHostname()
        log.info "-${c_purple}[nf-core/ampliconseq]${c_red} Pipeline completed with errors${c_reset}-"
    }
...
def nfcoreHeader() {
    ...
    ${c_blue}  | \\| |       \\__, \\__/ |  \\ |___     ${c_green}\\`-._,-`-,${c_reset}
                                            ${c_green}`._,._,\'${c_reset}
    ${c_purple}  nf-core/ampliconseq v${workflow.manifest.version}${c_reset}
    -${c_dim}--------------------------------------------------${c_reset}-
    """.stripIndent()
}
